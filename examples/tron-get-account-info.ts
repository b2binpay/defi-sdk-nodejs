/**
 * Tron: Get account info use case:
 * - Authenticate with API key only (no wallet required).
 * - Fetch account details and filter Tron deployments.
 * - For every Tron deployment, print balance summary and list non-zero asset balances.
 */
import 'dotenv/config';
import { AssetSortField, AssetSortOrder, DefiClient, FiatCurrency, TRON_CHAIN_IDS } from '../src';
...
runMain(async () => {
  ...
  const tronDeployments = deployments.filter((d) => TRON_CHAIN_IDS.has(Number(d.chainId)));

  if (tronDeployments.length === 0) {
    throw new Error('No Tron deployments found for this account.');
  }

  console.log(`Found ${tronDeployments.length} Tron deployment(s). Fetching balances...`);

  for (const deployment of tronDeployments) {
    const chainIdNumber = Number.parseInt(deployment.chainId, 10);
    console.log(`\nTron Chain ${deployment.chainId}`);
    ...
    if (nonZeroBalances.length === 0) {
      console.log('No non-zero balances on this Tron deployment.');
      continue;
    }
